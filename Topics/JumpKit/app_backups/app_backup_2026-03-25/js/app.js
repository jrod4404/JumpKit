...
notifBtn.addEventListener('click', () => {
  Modal.open('<i class="ti ti-bell"></i> Notifications',
    `<div style="text-align:center;padding:32px 0;color:var(--text-muted)">
      <i class="ti ti-bell-off" style="font-size:2.5rem;display:block;margin-bottom:12px"></i>
      <p>You're all caught up — no new notifications.</p>
    </div>`,
    `<button class="btn btn-subtle" onclick="Modal.close()"><i class="ti ti-x"></i> Close</button>`, 'sm');
});
...
function updateUserDisplay() {
  ...
  document.getElementById('userAvatar').textContent = fName.charAt(0).toUpperCase();
}
...
const pageTitles = {
  ...
  stats:'Statistics', settings:'Settings', help:'Help', account:'My Account', jet:'Jet AI', feedback:'Feedback', teams:'Teams'
};
const pageIcons = {
  home:'ti-home', jumps:'ti-run', archive:'ti-archive',
  stats:'ti-chart-bar', settings:'ti-settings', help:'ti-help-circle', account:'ti-user-circle', jet:'ti-brain', feedback:'ti-message-circle', teams:'ti-users'
};
let activePage = 'home';
...
function renderHome() {
  ...
  if (sub) sub.textContent = `${greeting}, ${currentUser.name.split(' ')[0]}!   Here's a few tips to get started, enjoy!`;
  document.getElementById('pageContent').innerHTML = `
    <div class="tips-grid">
      <div class="tip-card">
        <h3><span class="tip-icon"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 105.74 122.88" fill="currentColor" style="width:1.5rem;height:1.5rem;display:block"><path d="M3.07,79.92c4.32,1.19,29.57,17.12,32.69,10.85c0.32-0.64,2.87-6.24,2.87-6.27l13.62,3.47c0.44,1.39-5.97,12.95-7.23,14.27 c-1.6,1.68-3.21,2.68-4.93,3.57C34.31,108.79,6.82,94.12,0,93.16L3.07,79.92L3.07,79.92z M75.85,119.82 c0.63,0.24,0.89,1.1,0.58,1.93c-0.31,0.83-1.07,1.31-1.7,1.07l-18.78-7.03c-0.63-0.24-0.89-1.1-0.58-1.93 c0.31-0.83,1.07-1.31,1.7-1.07L75.85,119.82L75.85,119.82z M86.79,112.13c0.63,0.24,0.89,1.1,0.58,1.93 c-0.31,0.83-1.07,1.31-1.7,1.07l-18.78-7.03c-0.63-0.24-0.89-1.1-0.58-1.93s1.07-1.31,1.7-1.07L86.79,112.13L86.79,112.13z M87.12,100.47c0.63,0.24,0.89,1.1,0.58,1.93c-0.31,0.83-1.07,1.31-1.7,1.07l-18.78-7.03c-0.63-0.24-0.89-1.1-0.58-1.93 c0.31-0.83,1.07-1.31,1.7-1.07L87.12,100.47L87.12,100.47z M22.26,22.99c-0.66-0.15-1.03-0.97-0.83-1.83 c0.19-0.86,0.88-1.44,1.54-1.29l19.56,4.41c0.66,0.15,1.03,0.97,0.83,1.83c-0.19,0.86-0.88,1.44-1.54,1.29L22.26,22.99L22.26,22.99 z M19.79,12.13c-0.66-0.15-1.03-0.97-0.83-1.83c0.19-0.86,0.88-1.44,1.54-1.29l19.56,4.41c0.66,0.15,1.03,0.97,0.83,1.83 c-0.19,0.86-0.88,1.44-1.54,1.29L19.79,12.13L19.79,12.13z M25.69,3.15C25.03,3,24.66,2.18,24.85,1.32 c0.19-0.86,0.88-1.44,1.54-1.29l19.56,4.41c0.66,0.15,1.03,0.97,0.83,1.83c-0.19,0.86-0.88,1.44-1.54,1.29L25.69,3.15L25.69,3.15z M38.97,47.21l-2.86,17.67c-0.58,6.69-0.63,11.89,5.95,15c3.44,1.62,4.32,1.42,8.12,2.06l19.27-0.42 c1.04-0.02,26.34,11.02,28.43,12.43l7.83-9.36c1.1-1.31-25.7-14.04-29.63-15.46c-18.65-6.72-20.64,10.5-16.9-15.51 c3.75,2.9,6.93,3.62,13.62,5.39c8.01,1.1,11.41-0.86,17.65-3.7l9.22-4.57l-7.14-10.84l-7.05,4.2c-0.26,0.12-0.92,0.45-2.08,1.01 c-2.92,1.07-5.25,1.95-7.25,1.26c-6.64-2.32-12.06-12.07-29.81-11.45c-24.69,0.86-22.32-2.09-38.63,17.42l9.79,7.55 c7.7-9.21,8.39-11.43,20.79-12.61C38.52,47.24,38.74,47.23,38.97,47.21L38.97,47.21L38.97,47.21z M59.12,9.04 c6.83-3.12,14.89-0.11,18,6.72c3.12,6.83,0.11,14.89-6.72,18c-6.83,3.12-14.89,0.11-18-6.72C49.28,20.21,52.29,12.15,59.12,9.04 L59.12,9.04z"/></svg></span>Add Your First Jump</h3>
...
function renderSettings() {
  ...
  const pageChoices = ['home','jumps','stats','settings','help','jet'].map(pg =>
    ...
    <div class="acct-grid">
      <div class="acct-section">
        ...
        <div class="acct-row">
          ...
          <span style="font-size:0.88rem;color:var(--text-muted)">${esc(u.name)}</span>
        </div>
        <div class="acct-row">
          <div class="acct-row-label"><span>Email</span></div>
          <span style="font-size:0.88rem;color:var(--text-muted)">${esc(u.email)}</span>
        </div>
      </div>
      <div class="acct-section">
        ...
        <div class="acct-row">
          ...
          <span style="font-size:0.88rem;color:var(--text-muted)">Free</span>
        </div>
        <div class="acct-row">
        ...
        <div class="acct-row">
          <div class="acct-row-label"><span>Organization</span></div>
          <span style="font-size:0.88rem;color:var(--text-muted)">—</span>
        </div>
        <div class="acct-row">
          <div class="acct-row-label"><span>Team</span></div>
          <span style="font-size:0.88rem;color:var(--text-muted)">—</span>
        </div>
        <div class="acct-row">
          <div class="acct-row-label"><span>Role</span></div>
          <span style="font-size:0.88rem;color:var(--text-muted)">Owner</span>
        </div>
        <div class="acct-row" style="border-bottom:none">
          <div class="acct-row-label"><span>Member Since</span></div>
          <span style="font-size:0.88rem;color:var(--text-muted)">${new Date(u.createdAt).toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}</span>
        </div>
      </div>
      <div class="acct-save-row" style="justify-content:flex-start">
...
function wireDropdown({ dropId, triggerId, menuId, labelId, inputId = null, onSelect = null }) {
...
function saveAccountPrefs() {
  ...
  // Flash save button
  const btn = document.querySelector('.acct-save-row .btn-save');
  btn.innerHTML = '<i class="ti ti-circle-check"></i> Saved!';
  setTimeout(() => { btn.innerHTML = '<i class="ti ti-check"></i> Save Preferences'; }, 1800);
...
function renderStatsDash() {
  ...
  const mins   = Math.round((n * prefs.timePerClick) / 60);
  const dollars= ((n * prefs.timePerClick / 3600) * prefs.dollarsPerHour).toFixed(2);
...
window.showPaywall = function() {
    ...
    <div style="text-align:center;padding:16px 0">
      ...
      <div style="display:flex;flex-direction:column;gap:12px;max-width:280px;margin:0 auto">
        <button class="btn btn-primary" onclick="if (window.electronAPI) window.electronAPI.openUrl('https://jumpkit.app/#pricing'); Modal.close();" style="padding:14px;font-size:1rem;font-weight:700">
          <i class="ti ti-rocket"></i> Upgrade to JumpKit — $5/mo
        </button>
        <button class="btn btn-subtle" onclick="if (window.electronAPI) window.electronAPI.openUrl('https://jumpkit.app/#pricing'); Modal.close();" style="padding:12px;font-size:0.9rem">
          <i class="ti ti-sparkles"></i> JumpKit + Jet AI — $25/mo
        </button>
  ...
  `;
  Modal.open('<i class="ti ti-crown"></i> Upgrade JumpKit', body, '', { closeable: false });
};

  ...
  try {
    ...
    for (const rc of remoteColsDeduped) {
      ...
      const existing = existingCols.find(c =>
        ...
        (Array.isArray(c.sharedTeams) && c.sharedTeams.some(st => st.teamId === rc.team_id && st.supabaseId === rc.id)) ||
        (Array.isArray(c.sharedTeams) && c.sharedTeams.some(st => st.teamId === rc.team_id) && c.name === rc.name)
      );
      if (existing) {

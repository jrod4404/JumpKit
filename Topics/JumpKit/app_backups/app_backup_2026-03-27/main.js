    ...
    db.prepare(`
      INSERT OR REPLACE INTO jumps
        (id, userId, name, url, description, reason, columnId, hotkey, favorite,
         isArchived, clickCount, lastUsed, createdAt, updatedAt, isShared, teamId, timeSaved, timeSavedUnit)
      VALUES
        (@id, @userId, @name, @url, @description, @reason, @columnId, @hotkey, @favorite,
         @isArchived, @clickCount, @lastUsed, @createdAt, @updatedAt, @isShared, @teamId, @timeSaved, @timeSavedUnit)
    `).run({
...
ipcMain.handle('save-columns', (_e, userId, cols) => {
  ...
  try {
    const insert = db.prepare(`
      INSERT OR REPLACE INTO columns (id, userId, name, visible, \`order\`, createdAt, isShared, teamId)
      VALUES (@id, @userId, @name, @visible, @order, @createdAt, @isShared, @teamId)
    `);
    const tx = db.transaction(() => {
    ...
    db.prepare(`
      INSERT OR REPLACE INTO user_prefs
        (userId, startPage, timePerClick, dollarsPerHour, showDescription, showHotkey,
         subscriptionStatus, subscriptionTier, role, notifications, cloudBackup, autoArchive)
      VALUES
        (@userId, @startPage, @timePerClick, @dollarsPerHour, @showDescription, @showHotkey,
         @subscriptionStatus, @subscriptionTier, @role, @notifications, @cloudBackup, @autoArchive)
    `).run({
...
ipcMain.handle('seed-new-user', (_e, userId, platform) => {
  ...
  try {
    ...
    const insertCol = db.prepare(`
      INSERT OR REPLACE INTO columns (id, userId, name, visible, \`order\`, createdAt, isShared, teamId)
      VALUES (@id, @userId, @name, @visible, @order, @createdAt, 0, NULL)
    `);

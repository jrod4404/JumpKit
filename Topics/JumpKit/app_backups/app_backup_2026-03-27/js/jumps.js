...
function renderArchivedInline() {
  ...
  colEl.innerHTML = `
    ...
    <div class="col-items">${filtered.map(j => `
      <div class="jump-item" data-jid="${j.id}">
        <div class="jump-name">${esc(j.name)}</div>
        <div class="jump-url">${esc(j.url)}</div>
        <div style="display:flex;gap:6px;margin-top:6px">
          <button class="btn btn-subtle" style="font-size:0.78rem;padding:4px 10px" onclick="doUnarchive('${j.id}')"><i class="ti ti-refresh"></i> Restore</button>
...
function openJumpContextMenu(x, y, id) {
  ...
  if (isShared) {
    // Shared jumps: only allow hotkey edit
    items.push({ icon: '<i class="ti ti-keyboard"></i>', label: 'Set Hotkey', action: () => openHotkeyModal(id) });
    items.push({ icon: '<i class="ti ti-info-circle" style="color:var(--turq)"></i>', label: 'Shared — edit disabled', action: () => {} });
  } else {
...
function renderColConfigModal(cols) {
  const teamOptions = _colConfigTeams.length
    ? `<option value="">Personal only</option>` +
      _colConfigTeams.map(t => `<option value="${esc(t.id)}">${esc(t.name)}</option>`).join('')
    : null;
  ...
  const rows = cols.map((c, i) => `
    <div class="col-config-item" data-idx="${i}" draggable="true">
      ...
      ${showTeamShare ? `
      <select class="form-input" data-field="teamId" style="font-size:.78rem;padding:5px 8px;min-width:110px" title="Share with team">
        ${teamOptions.replace(
          `value="${esc(c.teamId || '')}"`,
          `value="${esc(c.teamId || '')}" selected`
        )}
      </select>` : ''}
    </div>`).join('');
...
async function saveColumns() {
  ...
  items.forEach((item, i) => {
    ...
    const order    = i + 1;
    const teamSel  = item.querySelector('[data-field="teamId"]');
    ...
    if (name) {
      ...
      if (existingCol && !existingCol._new) {
        ...
        if (isShared && !wasShared) toShare.push({ ...existingCol, name, visible, order, isShared: 1, teamId });
        if (wasShared && !isShared) toUnshare.push(existingCol);
...
async function syncColumnToSupabase(col) {
  try {
    ...
    // Upsert shared_column
    const { data: sc, error: scErr } = await supabaseClient
      .from('shared_columns')
      .upsert({
        id: col.id,  // use local id as stable key (UUID-ish)
        team_id: col.teamId,
        name: col.name,
        position: col.order,
...
async function unshareColumnFromSupabase(col) {
  try {
    ...
    await supabaseClient.from('shared_columns').delete().eq('id', col.id);

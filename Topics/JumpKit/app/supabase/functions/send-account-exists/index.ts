// ============================================================
// JumpKit — Send Account Already Exists Email Edge Function
// ============================================================
// Deploy: supabase functions deploy send-account-exists
// Trigger: called from auth.js when signUp() returns identities: []
//          (meaning the email is already registered with a confirmed account)
// Sends: branded email telling the user their account exists, with
//        sign-in and reset-password CTAs. UI never reveals whether
//        the email is registered (no enumeration risk).
// ============================================================
...
serve(async (req) => {
  ...
  try {
    ...
    const html = buildAccountExistsHTML({ email: esc(email) });
    ...
    const resendRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: email,
        subject: 'Sign in to your JumpKit account',
        html,
      }),
    });
...
function buildAccountExistsHTML({ email }: { email: string }): string {
  return `<!DOCTYPE html>
<html lang="en">
...
<body style="margin:0;padding:0;background:#0f1117;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:620px;margin:20px auto;background:#0E1827;border-radius:16px;overflow:hidden;border:1px solid rgba(255,255,255,0.09)">
    ...
    <tr><td style="padding:36px 40px">
      <h2 style="margin:0 0 16px;font-size:20px;color:#C8D6E8;font-weight:600">You already have a JumpKit account</h2>
      <p style="margin:0 0 20px;font-size:15px;color:#7A93B4;line-height:1.7">
        Someone just tried to create a new JumpKit account using <strong style="color:#C8D6E8">${email}</strong>. If that was you — good news, your account already exists!
      </p>
      <p style="margin:0 0 28px;font-size:15px;color:#7A93B4;line-height:1.7">
        Just sign in below, or reset your password if you've forgotten it.
      </p>
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 auto 16px">
        <tr><td align="center" width="260" style="width:260px;border-radius:10px;background:linear-gradient(135deg,#50CACC,#1A4FD6)">
          <a href="https://jumpkit.app" style="display:block;padding:6px 19px;color:#ffffff;font-weight:700;font-size:1rem;text-decoration:none;border-radius:10px;text-align:center;white-space:nowrap">
            <img src="https://jumpkit.app/email-icons/icon-login-white.png" width="27" height="27" style="vertical-align:middle;margin-right:8px;margin-bottom:2px" alt="" />Sign In to JumpKit
          </a>
        </td></tr>
      </table>
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 auto 32px">
        <tr><td align="center" width="260" style="width:260px;border-radius:10px;background:linear-gradient(135deg,#50CACC,#1A4FD6)">
          <a href="https://jumpkit.app/reset-password" style="display:block;padding:10px 19px;color:#ffffff;font-weight:700;font-size:1rem;text-decoration:none;border-radius:10px;text-align:center;white-space:nowrap">
      ...
      <p style="margin:0;font-size:13px;color:#4A6280;line-height:1.6">
        If you didn't try to create an account, you can safely ignore this email. Your account and password have not been changed.
      </p>
    </td></tr>

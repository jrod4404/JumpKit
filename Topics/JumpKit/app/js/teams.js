...
window.cancelInvite = async function(inviteId, email, teamName) {
  Modal.open(
    ...
    `<button class="btn btn-subtle" onclick="Modal.close()"><svg class="ti ti-x"><use href="img/tabler-sprite.svg#tabler-x"/></svg> Keep Invite</button>
     <button class="btn btn-subtle" style="color:#C8D6E8;background:rgba(180,60,60,0.18);border-color:rgba(180,60,60,0.25)" onclick="confirmCancelInvite('${esc(inviteId)}')"><svg class="ti ti-mail-off" style="color:#C8D6E8"><use href="img/tabler-sprite.svg#tabler-mail-off"/></svg> Cancel Invitation</button>`,
    'sm'
  );
};

...
function renderColumns() {
  ...
  columns.forEach((col, colIndex) => {
    ...
    const upgradeNudge = hiddenCount > 0 ? `
      <div style="margin:4px 6px 4px;padding:5px 8px;background:rgba(80,202,204,0.06);border:1px solid rgba(80,202,204,0.18);border-radius:7px;display:flex;align-items:center;gap:6px;overflow:hidden">
        <span style="font-size:0.72rem;color:var(--text-muted);white-space:nowrap;flex-shrink:0">+${hiddenCount} more</span>
        <button class="btn btn-subtle" style="font-size:0.68rem;padding:2px 7px;flex-shrink:0;white-space:nowrap;min-width:0" onclick="showUpgradeModal('Shared Jump Limit','Upgrade to JumpKit Core to see all shared jumps — unlimited teams, unlimited shared jumps, and unlimited launches.')">Unlock Core</button>
      </div>` : '';

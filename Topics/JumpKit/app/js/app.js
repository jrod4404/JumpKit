...
themeBtn.addEventListener('click', () => {
  ...
  if (window.activePage === 'admin' && typeof window.renderAdmin === 'function') {
    renderAdmin().catch(() => {});
  }
});
...
window.renderAccount = function renderAccount(initialTab = 'account') {
  ...
  const planBadge = _planLabel ? `<span style="background:${_planIsTest ? 'rgba(245,158,11,0.12)' : 'rgba(72,187,120,0.12)'};color:${_planIsTest ? '#d97706' : '#48BB78'};font-weight:600;font-size:0.72rem;padding:2px 8px;border-radius:20px;margin-left:6px">${_planLabel}</span>` : '';
...
window.runCloudBackup = async function runCloudBackup() {
  ...
  if (window.electronAPI?.saveBackup) {
    ...
    if (!confirmed) {
      setTimeout(() => window.Toast?.success('Auto-backup skipped.'), 1000);
      return;
    }

...
function renderPostMedia(mediaPathsStr) {
  ...
  const thumbs = paths.map(p => {
    ...
    if (isImg) return `<img src="/media/${p}" class="post-media-thumb" loading="lazy">`;
    if (isVid) return `<video src="/media/${p}" class="post-media-thumb" muted></video>`;
...
async function generatePostImage(postId) {
  try {
    ...
    const result = await apiFetch(`/posts/${postId}/generate-image`, { method: 'POST' });
...
function renderPlatformTabContent(posts) {
  ...
  el.innerHTML = platformPosts.map(post => {
    ...
    <div class="post-card card ${isScheduled ? 'post-card-scheduled' : ''}">
      ...
      <div class="post-card-actions">
        ...
        ${post.image_prompt && !hasImage ? `<button class="btn-gen-img btn-sm" onclick="generatePostImage('${post.id}')">🎨 Generate Image</button>` : ''}
        ...
        ${isScheduled ? `<span class="post-scheduled-confirm">✓ Scheduled</span>` : ''}
        <button class="btn-danger-subtle btn-sm" style="margin-left:auto" onclick="deleteGeneratedPost('${post.id}')" title="Delete post"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg></button>
      </div>
    </div>`;
  }).join('');
}
...
function loadSettings() {
  ...
  pane.innerHTML = `
    <div class="tab-header" style="justify-content:flex-start">
...
async function renderSettings() {
  ...
  <div class="settings-wrap">
    ...
    <!-- Config Row: App Settings + Hermes + Integrations -->
    ...
    <!-- Hermes Connection -->
    <div class="acct-section settings-col">
      <div class="acct-section-title"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"/><path d="M12 1v4M12 19v4M4.22 4.22l2.83 2.83M16.95 16.95l2.83 2.83M1 12h4M19 12h4M4.22 19.78l2.83-2.83M16.95 7.05l2.83-2.83"/></svg> Hermes Connection</div>
      <div class="acct-row acct-row-stacked">
        <div class="acct-row-label">
          <span>Config File Path</span>
          <span class="acct-row-hint">Path to Hermes social skill config JSON</span>
        </div>
        <div class="acct-control" style="min-width:300px">
          <input type="text" class="form-input" id="setting-hermes-path" value="${get('hermes.config_path')}" placeholder="/path/to/hermes/config.json" style="flex:1">
          <button class="btn-subtle" style="height:38px;padding:0 14px;flex-shrink:0" onclick="testHermesConnection()">Test</button>
        </div>
      </div>
      <div id="hermes-config-preview" style="display:none;padding:0 20px 16px">
        <div class="acct-row-hint" style="margin-bottom:6px">Hermes config (read-only)</div>
    ...
    <!-- Integrations -->
    <div class="acct-section settings-col">
      <div class="acct-section-title"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.72-1.71"/></svg> Integrations</div>
      <div class="acct-row acct-row-stacked">
        <div class="acct-row-label">
          <span>Image Generation API Key</span>
          <span class="acct-row-hint">OpenAI API key for DALL-E 3 image generation</span>
        </div>
        <div class="acct-control" style="min-width:300px">
          <input type="password" class="form-input" id="setting-image-gen-api-key" value="${get('image_gen.api_key') || ''}" placeholder="sk-..." oninput="markSettingDirty('image_gen.api_key', this.value)">
        </div>
      </div>
    </div>
    </div>
  ...
  setTimeout(() => initCustomSelects(el), 50);
}
...
async function saveSettings() {
  ...
  if (xClientSecretEl) payload['x.client_secret'] = xClientSecretEl.value.trim();
  const imgGenKeyEl = document.getElementById('setting-image-gen-api-key');
  if (imgGenKeyEl)     payload['image_gen.api_key'] = imgGenKeyEl.value.trim();
...
function injectToastStyles() {
  ...
  style.textContent = `
    ...
    /* Platform tabs */
    .generated-posts-header { font-size:13px; font-weight:700; color:var(--accent); margin-bottom:10px; }
    .platform-tabs { display:flex; gap:2px; margin-bottom:16px; background:var(--bg-surface); border-radius:8px; padding:3px; }
    ...
    .btn-publish { background:rgba(34,197,94,0.12); color:#22c55e; border:1px solid rgba(34,197,94,0.3); font-weight:600; cursor:pointer; border-radius:6px; transition:all .15s; }
    .btn-publish:hover { background:rgba(34,197,94,0.2); border-color:#22c55e; }
    ...
    .post-scheduled-confirm { font-size:12px; font-weight:600; color:var(--accent); display:inline-flex; align-items:center; gap:3px; }

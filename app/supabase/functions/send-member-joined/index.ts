// ============================================================
// JumpKit — Send Member Joined Edge Function
// ============================================================
// Deploy to Supabase Edge Functions:
//   supabase functions deploy send-member-joined
//
...
//
// The function:
//   1. Receives { ownerEmail, ownerName, memberName, memberEmail, teamName, totalMembers, joinedAt }
//   2. Sends a branded "member joined" notification email to the team owner via Resend
// ============================================================
...
serve(async (req) => {
  ...
  try {
    ...
    const { ownerEmail, ownerName, memberName, memberEmail, teamName, totalMembers, joinedAt } = await req.json();

    if (!ownerEmail || !memberName || !teamName) {
      return new Response(JSON.stringify({ error: 'ownerEmail, memberName, and teamName are required' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }

    const html = buildEmailHTML({ ownerName, memberName, memberEmail, teamName, totalMembers, joinedAt });
    ...
    const resendRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: ownerEmail,
        subject: `${memberName} just joined your team on JumpKit`,
        html,
      }),
    });
...
function buildEmailHTML({ ownerName, memberName, memberEmail, teamName, totalMembers, joinedAt }) {
  const firstName = ownerName ? ownerName.split(' ')[0] : 'there';
  const joinedAtFormatted = joinedAt
    ? new Date(joinedAt).toLocaleString('en-US', { month: 'long', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit', timeZoneName: 'short' })
    : 'just now';
...
<table role="presentation" align="center" cellpadding="0" cellspacing="0" width="620" style="max-width:620px;margin:20px auto;background:#0E1827;border-radius:16px;overflow:hidden;border:1px solid rgba(255,255,255,0.09)">
  ...
  <tr><td style="padding:36px 40px">
    <h2 style="margin:0 0 16px;font-size:20px;color:#C8D6E8;font-weight:600">A new member joined your team! 🎉</h2>
    <p style="margin:0 0 24px;font-size:15px;color:#7A93B4;line-height:1.6">
      Hi <strong style="color:#C8D6E8">${esc(firstName)}</strong>,
      <strong style="color:#C8D6E8">${esc(memberName)}</strong> has joined your team
    ...
    <div style="background:rgba(255,255,255,0.04);border-radius:10px;padding:20px 24px;border:1px solid rgba(255,255,255,0.06);margin-bottom:20px">
      <p style="margin:0 0 12px;font-size:14px;font-weight:600;color:#C8D6E8">Team snapshot:</p>
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
        <tr>
          <td style="padding:6px 0;font-size:13px;color:#7A93B4;line-height:1.5;width:40%"><span style="color:#50CACC;font-weight:600">Team</span></td>
          <td style="padding:6px 0;font-size:13px;color:#C8D6E8;line-height:1.5">${esc(teamName)}</td>
        </tr>
        <tr>
          <td style="padding:6px 0;font-size:13px;color:#7A93B4;line-height:1.5"><span style="color:#50CACC;font-weight:600">New member</span></td>
          <td style="padding:6px 0;font-size:13px;color:#C8D6E8;line-height:1.5">${esc(memberName)}${memberEmail ? ` <span style="color:#4A6280">(${esc(memberEmail)})</span>` : ''}</td>
        </tr>
        ${totalMembers ? `<tr>
          <td style="padding:6px 0;font-size:13px;color:#7A93B4;line-height:1.5"><span style="color:#50CACC;font-weight:600">Total members</span></td>
          <td style="padding:6px 0;font-size:13px;color:#C8D6E8;line-height:1.5">${esc(String(totalMembers))}</td>
        </tr>` : ''}
        <tr>
          <td style="padding:6px 0;font-size:13px;color:#7A93B4;line-height:1.5"><span style="color:#50CACC;font-weight:600">Joined at</span></td>
          <td style="padding:6px 0;font-size:13px;color:#C8D6E8;line-height:1.5">${esc(joinedAtFormatted)}</td>
        </tr>
      </table>
    </div>
    ...
    <div style="background:rgba(80,202,204,0.06);border-radius:10px;padding:16px 20px;border:1px solid rgba(80,202,204,0.15)">
      <p style="margin:0;font-size:13px;color:#7A93B4;line-height:1.6">
        They now have access to all shared columns and jumps on <strong style="color:#50CACC">${esc(teamName)}</strong>.
        You can manage team membership anytime from the <strong style="color:#C8D6E8">Teams</strong> page in JumpKit.
      </p>
    </div>
  </td></tr>
